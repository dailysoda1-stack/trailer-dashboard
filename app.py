...
/* Invisible Streamlit button stretched over thumbnail */
div[data-testid="stButton"].thumb-btn {
    position:absolute; inset:0; z-index:10;
}
div[data-testid="stButton"].thumb-btn button {
    position:absolute; inset:0;
    ...
    for row in grid_rows:
        ...
        for col, item in zip(cols, row):
            with col:
                ...
                # Thumbnail area — invisible Streamlit button overlaid on top
                thumb_container = st.container()
                with thumb_container:
                    # The visible card HTML (everything except the player)
                    close_lbl = "⏹ Close player" if is_playing else ""
                    st.markdown(f"""
                    <div class="{card_cls}">
                      <div class="vthumb" style="position:relative;">
                        {thumb_html}
                        {'<div class="play-overlay"><div class="play-circle"><div class="play-arrow"></div></div></div>' if video_id and not is_playing else ''}
                    ...
                    # Inline player replaces thumbnail when playing
                    if is_playing and video_id:
                        st.video(f"https://www.youtube.com/watch?v={video_id}")
                        if st.button("⏹ Close player", key=f"close_{key}", use_container_width=True):
                            st.session_state.playing = None; st.rerun()
                    elif video_id:
                        # Invisible full-width button that acts as the clickable thumbnail
                        st.markdown('<style>div[data-testid="stButton"]:last-child button{opacity:0;position:absolute;top:0;left:0;width:100%;height:200px;cursor:pointer;}</style>', unsafe_allow_html=True)
                        if st.button("▶", key=f"play_{key}", use_container_width=True):
                            st.session_state.playing = key; st.rerun()
